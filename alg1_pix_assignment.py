...
def recover_image_compare_orig_count(infile, slide_filenames, b_num = 2, b_pixels_to_flip = 100, m=1):
    ...
    for x in xrange(0, width):
        for y in xrange(0, height):
            ...
            for i, slide in enumerate(slide_images):
                # Count black pixels in the m×m block
                black_count = 0
                for mx in range(m):
                    for my in range(m):
                ...
                if i in byzantine_slides and pixels_flipped[i] < b_pixels_to_flip:
                    # Flip the entire m×m block
                    for mx in range(m):
                        for my in range(m):
                            current_value = slide.getpixel((x * m + mx, y * m + my))
                            slide.putpixel((x * m + mx, y * m + my), 1 - current_value)
                    pixels_flipped[i] += 1
                    flipped_pixels += 1
                
                # Add the majority value of the m×m block to pixels list
                pixels.append(0 if black_count > (m * m) // 2 else 1)
...
def encryptImage(infile, n_slides=2, m=1):
    ...
    for x in xrange(0, width):
        for y in xrange(0, height):
            ...
            # For black pixels: n-1 subpixels are black
            # For white pixels: 1 subpixel is black
            n_black = n_slides - 1 if pixel == 0 else 1
            ...
            # Assign one value to each slide and map to m×m block
            for i, draw in enumerate(draws):
                # Map the single pixel to an m×m block
                for mx in range(m):
                    for my in range(m):
                        draw.point((x * m + mx, y * m + my), subpixels[i])
...
def main():
    ...
    if args.comp1:
        ...
        black_pixels, white_pixels, black_percentage, white_percentage = recover_image_compare_orig_count(
            infile, slide_filenames, b_num=0, b_pixels_to_flip=0, m=m)
        ...
        for b_num, b_pixels in byzantine_configs:
            ...
            black_pixels, white_pixels, black_percentage, white_percentage = recover_image_compare_orig_count(
                infile, slide_filenames, b_num=b_num, b_pixels_to_flip=b_pixels, m=m)
        ...
        x_values = [f"({b_num},{b_pixels/img_size:.1%})" for b_num, b_pixels, _ in results]
        y_values = [black_percentage for _, _, black_percentage in results]
        
        plt.figure(figsize=(10, 6))
        plt.plot(x_values, y_values, 'bo-')
        plt.xlabel('Byzantine Configuration (b_num, affected pixels %)')
        plt.ylabel('Black Pixel Percentage')
        plt.title('Effect of Byzantine Participants on Black Pixels Recovery')
        plt.grid(True)
        plt.xticks(rotation=45)
        # Add text labels for black pixel percentages with increased visibility
        for i, black_percentage in enumerate(y_values):
            plt.text(i, black_percentage + 0.0007, f'{black_percentage:.2%}', 
                    ha='center', va='bottom', fontsize=10, 
                    bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=1))
        # Add a red horizontal line at y_values[0] height, from 0 to max x_values, with label
