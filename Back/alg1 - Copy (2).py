...
def recover_image_compare_orig_count(infile, slide_filenames, b_num = 2, b_pixels_to_flip = 100):
    ...
    for x in xrange(0, width):
        for y in xrange(0, height):
            ...
            # If a byzantine slide flips a pixel, it affects the count
    ...
    print(f"Black pixels: {black_pixels} ({black_pixels/orig_black:.2%})")
    print(f"White pixels: {white_pixels} ({white_pixels/orig_white:.2%})")
...
def main():
    ...
    if args.comp1:
        ...
        # --- 1. Vary b_num, keep b_pixels fixed ---
        fixed_b_pixels = args.b_pixels
        b_num_results = []
        for b_num in args.b_num:
            print(f"\nTesting with b_num={b_num}, b_pixels={fixed_b_pixels}")
            black_pixels, white_pixels, black_percentage, white_percentage = recover_image_compare_orig_count(
                infile, slide_filenames, b_num=b_num, b_pixels_to_flip=fixed_b_pixels)
            b_num_results.append((b_num, fixed_b_pixels, black_percentage))

        # Plot for varying b_num
        x_values = [str(b_num) for b_num, _, _ in b_num_results]
        y_values = [black_percentage for _, _, black_percentage in b_num_results]

        plt.figure(figsize=(10, 6))
        plt.plot(x_values, y_values, 'bo-')
        plt.xlabel('Number of Byzantine Slides (b_num)')
        plt.ylabel('Black Pixel Percentage')
        plt.title('Effect of Number of Byzantine Slides on Black Pixels Recovery')
        plt.grid(True)
        plt.xticks(rotation=45)
        for i, black_percentage in enumerate(y_values):
            plt.text(i, black_percentage + 0.0007, f'{black_percentage:.2%}',
                     ha='center', va='bottom', fontsize=10,
                     bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=1))
        plt.axhline(y=y_values[0], color='red', linestyle='--', linewidth=2, label='Wave Interference VES')
        plt.legend()
        plt.gca().yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1, decimals=1))
        plt.tight_layout()
        plt.savefig(f"{base_out_dir}byzantine_effect_bnum.png")
        plt.close()

        # --- 2. Vary b_pixels, keep b_num fixed ---
        fixed_b_num = args.b_num[0]
        # Use a default range for b_pixels: 0, 1/4, 1/2, 3/4, 1 of the image size
        total_pixels = slide_images[0].size[0] * slide_images[0].size[1]
        b_pixels_range = [int(x) for x in np.linspace(0, total_pixels, 6)]

        b_pixels_results = []
        for b_pixels in b_pixels_range:
            print(f"\nTesting with b_num={fixed_b_num}, b_pixels={b_pixels}")
            black_pixels, white_pixels, black_percentage, white_percentage = recover_image_compare_orig_count(
                infile, slide_filenames, b_num=fixed_b_num, b_pixels_to_flip=b_pixels)
            b_pixels_results.append((fixed_b_num, b_pixels, black_percentage))

        # Plot for varying b_pixels
        x_values = [str(b_pixels) for _, b_pixels, _ in b_pixels_results]
        y_values = [black_percentage for _, _, black_percentage in b_pixels_results]

        plt.figure(figsize=(10, 6))
        plt.plot(x_values, y_values, 'bo-')
        plt.xlabel('Number of Pixels Flipped (b_pixels)')
        plt.ylabel('Black Pixel Percentage')
        plt.title('Effect of Number of Pixels Flipped on Black Pixels Recovery')
        plt.grid(True)
        plt.xticks(rotation=45)
        for i, black_percentage in enumerate(y_values):
            plt.text(i, black_percentage + 0.0007, f'{black_percentage:.2%}',
        ...
        plt.gca().yaxis.set_major_formatter(mtick.PercentFormatter(xmax=1, decimals=1))
        plt.tight_layout()
        plt.savefig(f"{base_out_dir}byzantine_effect_bpixels.png")
        plt.close()
