...
def encryptImage(infile, n_slides=2):
    ...
    # Generate all patterns with floor(N/2) black and the rest white
    k = n_slides // 2
    patterns = list(itertools.permutations([1]*k + [0]*(n_slides-k)))
    ...
    for x in xrange(0, int(width/2)):
        for y in xrange(0, int(height/2)):
            ...
            for i, draw in enumerate(draws):
                if pixel == 0:  # Dark pixel so slides get anti pattern
                    draw.point((x*2, y*2), 1-pat[i])
                    draw.point((x*2+1, y*2), 1-pat[i])
                    draw.point((x*2, y*2+1), 1-pat[i])
                    draw.point((x*2+1, y*2+1), 1-pat[i])
                else:
                    draw.point((x*2, y*2), pat[i])
                    draw.point((x*2+1, y*2), pat[i])
                    draw.point((x*2, y*2+1), pat[i])
                    draw.point((x*2+1, y*2+1), pat[i])
    # Save all slides
...
#    print("f:{}".format(f))ncaieee_ext
    ...
    slide_filename_A = slides_dir + f + "_slideA"
    slide_filename_B = slides_dir + f + "_slideB"
